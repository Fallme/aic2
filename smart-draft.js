// smart-draft.js — v4: Robust workflow with proper error handling
...
let mainFiles = [];
let isReady = false; // Track if session is ready for chat
...
function addFiles(files) {
  for (const f of files) {
    // Deduplicate
    if (!mainFiles.find(x => x.name === f.name)) {
      mainFiles.push({ name: f.name, file: f });
    }
  }
  renderAttached();
}
...
async function startDraft() {
  ...
  try {
    ...
    showStep("done");
    addBubble("assistant", `合同草稿已生成。你可以在右侧对话中补充信息，AI 会帮你完善合同。如需修改请直接告诉我。`);
    ...
    showStep("done");
    document.getElementById("docxContent").innerHTML = `<div style="padding:40px;text-align:center;color:var(--red)"><strong>生成失败</strong><br>${esc(e.message)}<br><br><button class="btn btn-primary btn-sm" onclick="startDraft()">重试</button></div>`;
  }
  isProcessing = false;
...
function showWordLoading(text) {
  document.getElementById("docxContent").innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;padding:60px">
    <div style="width:48px;height:48px;border:3px solid #e5e5e5;border-top-color:var(--gold);border-radius:50%;animation:spin .8s linear infinite;margin-bottom:20px"></div>
...
function addTyping() {
  const t = document.getElementById("chatThread");
  const d = document.createElement("div");
  d.className = "chat-msg assistant"; d.id = "typingDot";
...
async function sendAnswer() {
  ...
  try {
    ...
    if (data.dialogue?.missingCount > 0) {
      ...
      addBubble("assistant", "信息已齐全。可以点击生成草稿。");
    }
  } catch(e) { removeTyping(); addBubble("assistant", "错误: " + e.message); }
}
...
async function generateDraft() {
  ...
  try {
    ...
    const txt = await res.text();
    let data; try { data = JSON.parse(txt); } catch { throw new Error("生成失败"); }
    if (data.error) throw new Error(data.error);
